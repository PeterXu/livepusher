...
void* publiser(void *args) {
    ...
    do {
        ...
        if (!s_pusher.proto.rtmp) {
            throwNativeInfo(s_jni.errorId, E_RTMP_ALLOC);
            goto END;
        }
        ...
        if (!RTMP_Connect(s_pusher.proto.rtmp, NULL)) {
            throwNativeInfo(s_jni.errorId, E_RTMP_CONNECT);
            goto END;
        }
        ...
        if (!RTMP_ConnectStream(s_pusher.proto.rtmp, 0)) {
            throwNativeInfo(s_jni.errorId, E_RTMP_STREAM);
            goto END;
        }
        ...
        while (s_pusher.publishing) {
            ...
            if (packet) {
                ...
                if (!i) {
                    ...
                    throwNativeInfo(s_jni.errorId, E_RTMP_SEND);
                    goto END;
                }
                RTMPPacket_Free(packet);
            }
...
void setVideoOptions(video_enc_t *video, jint width, jint height, jint bitrate, jint fps) {
    ...
    if (!video->handle) {
        ...
        throwNativeInfo(s_jni.errorId, E_VIDEO_ENCODER);
        return;
    }
